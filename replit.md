...
- **Conversion Optimization**: Homepage enhancements including Before/After gallery (multi-image carousel with real and stock photos), trust badges, optimized CTAs, Google Reviews display with mascot accent, Exit-Intent Popup (active on homepage, 10% discount offer with email capture), Hero Urgency Chip, and a Booking Incentive Engine. Mascot (Self-Maid superhero character with transparent background) appears in quote calculator (with speech bubble), reviews section, and final CTA section to guide users toward requesting a quote. Micro-trust signals ("5.0 Google Rating · 500+ Homes Cleaned · Fully Insured") placed directly below CTA buttons in the quote calculator (both steps) and booking form. Phone field in calculator promoted with icon + "We'll text your quote summary instantly" helper text. Quote-to-booking flow passes frequency + propertySize via URL params for richer pre-fill and quote summary on booking page. Post-first-cleaning recurring upsell email sent 2 hours after job completion via `server/hooks/recurring-upsell.ts` (biweekly 10% / monthly 5% offers). Homepage section order: Hero → Calculator → TrustBar → BeforeAfterGallery → Services → GoogleReviews → FinalCTA → Contact → Footer.
