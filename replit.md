...
- **Homepage Inline Quote Calculator**: Replaced QuoteAdBanner with a fully functional inline quote calculator (`HomepageQuoteCalculator`). Two-step flow: Step 1 collects service type, bedrooms, bathrooms, pets, frequency and displays instant price; Step 2 captures name/email/phone, saves quote via POST /api/quotes (creates CRM lead with frequency and pet surcharge data), then redirects to /booking with all quote context. Located after Hero section. CTA reads "Book This Cleaning" when price is calculated.
...
- **Prattville Neighborhood SEO Pages**: 5 dedicated neighborhood pages at `/services/prattville/{neighborhood}` — Glennbrooke, Silver Hills, Highland Ridge, Hunting Ridge, Riverchase. Each has unique localized content, JSON-LD structured data, trust badges, service cards, customer reviews, nearby neighborhood links, and dual CTA sections. City parameter validated to prevent duplicate content. All added to sitemap.xml.
- **Expanded Prattville Neighborhoods**: City-service and service-area pages updated with 4 additional neighborhoods (Silver Hills, Highland Ridge, Hunting Ridge, Riverchase).
...
- **CRM Module** (`server/crm/`): Full CRM backend with routes (`/api/crm/*`), pipeline management, scheduling, lead-to-client conversion. Frontend at `/crm/*` with dashboard, pipeline board, contacts, appointments, jobs, and analytics pages. Pipeline stages: new_lead → quote_requested → contacted → qualified → proposal_sent → negotiation → won/lost → converted. Quote-to-lead automation: POST /api/quotes also creates a CRM lead with stage `quote_requested`. Dashboard tracks Total Quotes, Quote Requests, and Quote→Booking conversion rate.
