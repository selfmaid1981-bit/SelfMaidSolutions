...
- **Homepage Inline Quote Calculator**: Replaced QuoteAdBanner with a fully functional inline quote calculator (`HomepageQuoteCalculator`). Two-step flow: Step 1 collects service type, bedrooms, bathrooms, optional sqft and displays instant price; Step 2 captures name/email/phone, saves quote via POST /api/quotes (creates CRM lead), then redirects to /booking with all quote context. Located after HowItWorks section.
