...
## UI/UX Decisions
The homepage uses a clean, luxurious light aesthetic: cream (#F7F7F5) backgrounds, white (#FFFFFF) section alternation, navy (#1F2A37) text, muted gold (#C6A969) accents (light: #D4BC85, dark: #A88B4A). Typography uses Cormorant Garamond (serif headings) and Inter (body text). The homepage layout follows: Sticky Light Header → Hero (kitchen bg at low opacity, centered logo, pill CTA, stats bar) → Services (6-card grid) → How It Works (3 steps) → Quote Calculator (light card) → CTA Band (navy bg) → Testimonials Carousel → About (image + bio) → Areas (chip grid) → FAQ (native details) → Contact (form panel) → Footer (navy bg, 4-col) → Mobile CTA Bar (bottom fixed, light). All CSS variables are prefixed with --sm- in index.css. No dark top bar. No gold particles. Admin/CRM/portal pages use Radix UI/shadcn components.
...
- **CRM Module**: Full CRM backend with routes, pipeline management, scheduling, lead-to-client conversion, and dashboard metrics. Integrated booking calendar at `/crm/calendar` shows month-view grid combining bookings + appointments with color-coded events (blue=bookings, green=appointments), day click detail panel, event detail sidebar with customer contact info (click-to-call/email), month revenue stats. API: `GET /api/crm/calendar` merges data from bookings and appointments tables.
...
- **Ad Automation**: AI-powered ad content generation for Facebook, Instagram, YouTube, and TikTok at `/admin/ads`. Generates platform-specific ad copy (headlines, captions, CTAs, hashtags, target audiences) using OpenAI. Single ad generation + batch generation across multiple platforms. Ad library with status tracking (draft → ready → published). Copy-to-clipboard for easy posting. Module: `server/ad-automation.ts`, page: `client/src/pages/admin/ad-automation.tsx`, table: `ad_campaigns`.
- **Weekly Schedule Email**: Print-friendly weekly schedule sent every Sunday at 6 PM CT to owner emails. Day-by-day breakdown with customer name, phone, email, address, service type, time, amount, status, and notes. Summary stats (total jobs, active days, estimated revenue). Color-coded booking vs appointment legend. Manual trigger: `POST /api/admin/weekly-schedule`. Module: `server/weekly-schedule-email.ts`.
...
- **Real-Time Availability Calendar**: Booking page and booking modal show available time slots (8 AM, 10 AM, 12 PM, 2 PM, 4 PM) for selected dates. Slots check bookings + appointments tables. Up to 2 bookings per slot allowed. API: `GET /api/availability?date=YYYY-MM-DD`. Component: `client/src/components/availability-picker.tsx`.
- **Customer Portal**: Email-based login at `/portal` — customers enter their email to view booking history, upcoming appointments, total spent, loyalty status (Member/Gold/VIP), and rebook with pre-filled info. API: `POST /api/portal/login`, `GET /api/portal/customer?email=`. Page: `client/src/pages/customer-portal.tsx`. Footer "My Bookings" link.
- **Review Follow-Up Automation**: 3-day follow-up reminder auto-scheduled after initial review request. Re-sends SMS + email if customer hasn't left a review. Functions: `scheduleReviewFollowUp()`, `sendReviewFollowUp()` in `server/hooks/review-on-complete.ts`.
