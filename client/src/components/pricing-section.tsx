    ...
    <section className="py-20 lg:py-28 marble-bg dark:bg-slate-900/40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {services.map((service) => (
            <div
              ...
              className={`relative rounded-2xl p-6 border transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${
                service.highlight
                  ? 'bg-gradient-to-br from-[#1F2A37] to-[#0c2e35] border-[#C6A969]/40 text-white shadow-xl shadow-[#0c2e35]/30'
            ...
            >
              ...
              <p className={`text-sm mb-3 ${service.highlight ? 'text-[#C6A969]' : 'text-muted-foreground'}`}>
                {service.tagline}
              </p>

              <div className={`text-2xl font-extrabold mb-4 ${service.highlight ? 'text-[#C6A969]' : 'text-[#1F2A37] dark:text-[#C6A969]'}`}>
                {service.range}
              </div>
              ...
              <ul className="space-y-2 mb-6">
                {service.features.map((feat) => (
                  <li key={feat} className="flex items-center gap-2 text-sm">
                    <CheckCircle className={`w-4 h-4 flex-shrink-0 ${service.highlight ? 'text-[#C6A969]' : 'text-[#C6A969]'}`} />
                    <span className={service.highlight ? 'text-white/80' : 'text-foreground/75'}>{feat}</span>
                  </li>
                ))}
              </ul>
        ...
        <div className="text-center">
          ...
          <Link href="/quote">
            <Button size="lg" className="bg-[#1F2A37] text-[#C6A969] text-lg px-10 py-6 font-bold rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 hover:bg-[#2a3a4d] transition-all duration-300">
