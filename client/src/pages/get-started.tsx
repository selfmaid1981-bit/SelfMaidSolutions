    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 px-4">
        <div className="text-center max-w-lg">
          <div className="w-20 h-20 gold-bg rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl">
          ...
          <a
            ...
            className="inline-flex items-center gap-3 gold-bg text-[#1F2A37] px-8 py-4 rounded-2xl font-bold text-lg shadow-xl"
          >
      ...
      <main className="max-w-2xl mx-auto px-4 pb-16 pt-8">
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl sm:text-5xl font-bold text-white mb-4 leading-[1.1]">
            Get a <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#C6A969] to-[#F5D76E]">Spotless Home</span><br />Without the Stress
          </h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {trustBadges.map(({ icon: Icon, label }) => (
            <div key={label} className="bg-white/8 border border-white/10 rounded-xl p-3 text-center">
              <Icon className="w-5 h-5 text-[#C6A969] mx-auto mb-1.5" />
        ...
        <div className="bg-white/[0.06] border border-white/10 backdrop-blur-sm rounded-3xl p-6 sm:p-8 shadow-2xl mb-6">
          <div className="h-1 gold-bg rounded-full mb-6 -mx-6 sm:-mx-8 mt-[-24px] sm:mt-[-32px] rounded-t-3xl" />
            ...
            <form onSubmit={form.handleSubmit((data) => mutation.mutate(data))} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <FormField control={form.control} name="name" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder="Jane Smith" className="bg-white/10 border-white/20 text-white placeholder:text-slate-500 rounded-xl h-11 focus:border-[#C6A969] focus:bg-white/15" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="phone" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} type="tel" placeholder="(334) 555-0000" className="bg-white/10 border-white/20 text-white placeholder:text-slate-500 rounded-xl h-11 focus:border-[#C6A969] focus:bg-white/15" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
              ...
              <FormField control={form.control} name="email" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input {...field} type="email" placeholder="you@email.com" className="bg-white/10 border-white/20 text-white placeholder:text-slate-500 rounded-xl h-11 focus:border-[#C6A969] focus:bg-white/15" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="serviceType" render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} value={field.value}>
                    <FormControl>
                      <SelectTrigger className="bg-white/10 border-white/20 text-white rounded-xl h-11 focus:border-[#C6A969] focus:bg-white/15">
              ...
              <Button
                ...
                className="w-full gold-bg hover:opacity-90 text-[#1F2A37] font-bold py-6 rounded-2xl text-lg shadow-xl hover:shadow-2xl transition-all h-auto mt-2"
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8">
          {[
            ...
            <div key={text} className="flex items-center gap-2 bg-[#C6A969]/10 border border-[#C6A969]/20 rounded-xl p-3">
              <span className="text-[#C6A969] font-bold text-sm">{emoji}</span>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-4 flex gap-3">
              <div className="w-10 h-10 rounded-full gold-bg flex items-center justify-center flex-shrink-0 font-bold text-[#1F2A37] text-sm">
                {r.name[0]}
              </div>
              <div>
                ...
                <span className="text-[#C6A969] text-xs mt-1 block">{r.service}</span>
              </div>
            </div>
          ))}
        </div>
